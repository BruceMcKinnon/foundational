...
jQuery(document).ready(function () {
  ...
  jQuery("#back-top").hide(); // fade in #back-top
...
jQuery(".closebtn").bind("click", function () {
  console.log("User clicked on closebtn");
  closeMobileMenu();
});
...
function doAnchorScroll(e, id) {
  ...
  try {
    ...
    var target = jQuery(id.hash); //console.log( 'target:'+target);

    target = target.length ? target : $('[name=' + this.hash.slice(1) + ']');
  } //console.log('hash='+target);


  var pos = jQuery(target).offset().top; // If the sticky main nav will be visible, scroll down a bit so you can see
